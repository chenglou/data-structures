...
class Trie
	...
	add: (word) ->
		...
		currentNode = @_root
		for letter in word
			if not currentNode[letter]?
				currentNode[letter] = {}
			currentNode = currentNode[letter]
		# Add the terminator. The value should be something defined, as to not cause search problems
		# and so that we can validate using `if currentNode[WORD_END]` rather than `if currentNode[WORD_END]?`
		currentNode[WORD_END] = 1
		return word
